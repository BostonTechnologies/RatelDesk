...
public sealed class ChatMigrationTests : IAsyncLifetime
{
    ...
    {
        var options = new DbContextOptionsBuilder<HelpdeskDbContext>().UseNpgsql(postgres.GetConnectionString(), x => x.UseVector()).Options;
        await using var db = new HelpdeskDbContext(options, Substitute.For<ITenantContext>(), new HttpContextAccessor());
        await db.Database.ExecuteSqlRawAsync("CREATE EXTENSION IF NOT EXISTS \"uuid-ossp\"; CREATE EXTENSION IF NOT EXISTS vector;");
