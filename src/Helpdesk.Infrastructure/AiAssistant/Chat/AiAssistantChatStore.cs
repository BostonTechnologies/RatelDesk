...
public sealed class AiAssistantChatStore(HelpdeskDbContext db, ITenantContext tenant, IDomainEventPublisher domainEvents, ICorrelationContext correlation, ILogger<AiAssistantChatStore> logger) : IAiAssistantChatStore
{
    ...
    public async Task RequestRecoveryAsync(string type, string ticketId, Guid id, string actor, CancellationToken ct)
    {
        ...
        Append(db, conversation, "recovery_requested", "Operator requested session reconciliation without resending the message.", actor);
    ...
    public async Task AbandonAsync(string type, string ticketId, ChatAbandonRequest request, string actor, CancellationToken ct)
    {
        ...
        Append(db, conversation, "delivery_abandoned", "Operator acknowledged that the old AiAssistant turn may have been admitted and may still execute. This transcript is archived; no message was retried.", actor, request.ResolutionId);
        conversation.State = ChatState.Archived;
        Append(db, conversation, "archived", "Conversation archived after deliberate abandonment.", actor);
        ...
        await db.SaveChangesAsync(ct);
        db.Add(next);
        Append(db, next, "created", "New conversation started. No previous message has been resent.", actor);
        await db.SaveChangesAsync(ct);
        await tx.CommitAsync(ct);
    ...
    public static void Append(HelpdeskDbContext db, AiAssistantChatConversation conversation, string type, string text, string? actor = null, Guid? messageId = null, string? callId = null, string? options = null)
    {
        conversation.LastActivityUtc = DateTimeOffset.UtcNow;
        db.Set<AiAssistantChatEvent>().Add(new() { ConversationId = conversation.Id, Sequence = ++conversation.LastSequence, Type = type, Text = text, CreatedByUserId = actor, ClientMessageId = messageId, CallId = callId, OptionsJson = options });
    }

    public static void AppendActivity(HelpdeskDbContext db, AiAssistantChatConversation conversation, string type,
        string? callId, ChatActivityMetadata metadata)
    {
        conversation.LastActivityUtc = DateTimeOffset.UtcNow;
    ...
    public async Task<ChatSnapshot> LoadAsync(string type, string ticketId, Guid? conversationId, long after, string actor, CancellationToken ct)
    {
        ...
        if (conversation is null)
        {
            ...
            if (conversation is null)
            {
                ...
                db.Add(conversation);
                Append(db, conversation, "created", "Conversation started.", actor);
                await db.SaveChangesAsync(ct);
            }
            await tx.CommitAsync(ct);
        }
    ...
    public async Task<bool> AcceptMessageAsync(string type, string ticketId, ChatMessageRequest request, string actor, CancellationToken ct)
    {
        ...
        Append(db, conversation, "operator", request.Text, actor, request.ClientMessageId);
        await db.SaveChangesAsync(ct);
        await tx.CommitAsync(ct);
    ...
    public async Task AcceptApprovalAsync(string type, string ticketId, string callId, ChatApprovalRequest request, string actor, CancellationToken ct)
    {
        ...
        Append(db, conversation, "approval_response", request.SelectedKey, actor, callId: callId);
        await db.SaveChangesAsync(ct);
        await tx.CommitAsync(ct);
    ...
    public async Task<ChatSnapshot> NewAsync(string type, string ticketId, Guid id, string actor, CancellationToken ct)
    {
        ...
        await using (var tx = await db.Database.BeginTransactionAsync(ct))
        {
            ...
            Append(db, old, "archived", "Conversation archived.", actor);
            await db.SaveChangesAsync(ct);
            await tx.CommitAsync(ct);
        }
