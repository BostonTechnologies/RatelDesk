...
public sealed class AiAssistantChatOptions
{
    ...
    public bool IsValid()
    {
        ...
        if (Instance != "dev" || string.IsNullOrWhiteSpace(DeviceToken) || IdleMinutes <= 0 || ConnectionCapacity <= 0 || !Uri.TryCreate(Endpoint, UriKind.Absolute, out var uri) || uri.AbsolutePath != "/hub/session" || uri.UserInfo.Length != 0 || uri.Query.Length != 0) return false;
