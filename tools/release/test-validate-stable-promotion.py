...
class StablePromotionValidatorTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.release = {"tagName": TAG, "isDraft": False, "isPrerelease": False, "assets": [{"name": name} for name in archives() | {"SHA256SUMS", "release-manifest.json", "release-manifest.json.sha256"}]}
    ...
    def run_helper(self) -> subprocess.CompletedProcess[str]:
        paths = {"release": self.directory / "release.json", "manifest": self.directory / "release-manifest.json", "pages": self.directory / "pages.json", "inspections": self.directory / "inspections.json"}
        ...
        return subprocess.run(
            [sys.executable, str(HELPER), "--tag", TAG, "--registry-owner", "public-owner", "--source-revision", REVISION, "--release", str(paths["release"]), "--manifest", str(paths["manifest"]), "--manifest-checksum", str(checksum), "--release-pages", str(paths["pages"]), "--inspections", str(paths["inspections"])],
            cwd=self.directory,
            text=True,
            capture_output=True,
            check=False,
    ...
    def test_valid_plan_runs_from_empty_non_git_directory_and_is_idempotent(self) -> None:
    ...
    def test_rejects_missing_asset_altered_checksum_and_older_release(self) -> None:
